...
```{r}
encounter_rv_cv_pairs_conf = readRDS("encounter_data_pairs.rds")
...
summary_data <- DtAdjusted %>%
  pivot_longer(cols = starts_with("present_"), 
               names_to = "organism", 
               values_to = "present") %>%
  mutate(organism = gsub("present_", "", organism))


# Calculate prevalence and positive percentage
summary_data <- summary_data %>%
  group_by(date, organism) %>%
  summarise(
    n = sum(present),
    total_count = n_distinct(sample),
    prevalence = n / n_distinct(individual),  # Prevalence calculation
    positive_percentage = n / total_count    # Positive percentage calculation
  ) %>%
  ungroup() %>%
  mutate(date = as.Date(date))

# Modify organism labels
...
# Plot the prevalence
prevalence_plot <- ggplot(summary_data, aes(x = date, y = prevalence, fill = factor(organism))) +
  geom_area(stat = "smooth", method = "loess", position = "fill", alpha = 0.9, span = 1/4) +
  scale_fill_manual(name = "Organism",
                    values = c(
                      "Rhinovirus" = "#355070",
                      "SARS-CoV-2" = "#F94144", 
                      "RSV A" = "#F8961E",  
  ...
  scale_y_percent(scale = 100, limits = c(0, 100))

# Plot the positive percentage
ggplot(summary_data, aes(x = date, y = positive_percentage, fill = factor(organism))) +
  geom_area(stat = "smooth", method = "loess", alpha = 0.9, span = 1/4)+
  labs(x = "Date", y = "Positive Percentage")
```
...
length(unique(Rhinovirus_TimeDependent_cov$Rhinovirus_TimeDependent))
...
proportion_data <- Rhinovirus_TimeDependent_cov %>%
  #filter(Rhinovirus_TimeDependent != "No") %>%
  filter(present_rv_n == 1) %>%
  group_by(Rhinovirus_TimeDependent, present_rv_n, Secondary_Group) %>%
  summarise(count = n()) %>%
  #ungroup() %>%
  #group_by(Rhinovirus_TimeDependent, present_rv_n) %>%
  mutate(total = sum(count),
         proportion = count / total)

# Step 2: Plot the data
ggplot(proportion_data, aes(x = Rhinovirus_TimeDependent, y = proportion, color = as.factor(Secondary_Group), group = Secondary_Group)) +
  geom_line() +
  geom_point() +
  sm_corr_theme()
  #labs(x = "Rhinovirus Time Dependent", y = "Proportion of Secondary Group", color = "Presence of RV") +
```
...
Covid_TimeDependent_cov <- Covid_TimeDependent %>%
  left_join(DtAdjusted %>% 
              dplyr::select(individual,sample, agebin, sex,ct_Adenovirus, ct_IAV,ct_IBV, ct_RSV, ct_Rhinovirus, ct_SARS_CoV_2, date, symptoms), by = c("individual","sample")) %>%
  mutate(agebin = case_when(
    agebin %in% c("<1", "1-5") ~ "< 5",
    TRUE ~ as.character(agebin)
  )) %>%
...
# RSV primary unadjusted
fomular_unadj <- Surv(start_adjusted, stop_adjusted, Secondary_Group) ~ RSV_TimeDependent

# Formula RSV adjusted
...
# IAV primary unadjusted
fomular_unadj <- Surv(start_adjusted, stop_adjusted, Secondary_Group) ~ IAV_TimeDependent

# Formula IAV adjusted
...
#   
# 
# # IBV primary unadjusted
# fomular_unadj <- Surv(start_adjusted, stop_adjusted, Secondary_Group) ~ IBV_TimeDependent
# 
# # Formula IBV adjusted
...
Covid_TimeDependent_step1 <- Covid %>%
  filter(present_sars_cov_2_n == 1 & stop_adjusted - start_adjusted <= 14) %>%
  dplyr::select(individual) %>%
  distinct()
...
Covid_TimeDependent_step2 <- Covid_TimeDependent_step1 %>%
  filter(present_sars_cov_2_n == 0 & stop_adjusted - start_adjusted < 14)
...
baselineHazard <- coxph(Surv(start_adjusted, stop_adjusted, Secondary_Group) ~ Rhinovirus_TimeDependent, cluster = individual, data = Rhinovirus_TimeDependent_cov)
...
p <- 
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1000))+
                     #, breaks = xlab, labels = xlab) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.04),
                     #, breaks = ylab, labels = round(ylab, 3), 
...
### (6.1) Kaplan M curves showing non-proportional hazard

```{r}

# Formula Rhinovirus adjusted
fomular <- Surv(start_adjusted, stop_adjusted, present_rv_new) ~ present_sars_cov_2_n + sex+ agebin + symptoms
# Rhinovirus model
model <- coxph(fomular,cluster = individual, data = Covid_TimeDependent_cov)
summary(model)
...
survival_function_Cov <- function(data_x, strata_x) {
  # Create survival object
  data_x$s <- Surv(data_x$start_adjusted, data_x$stop_adjusted, data_x$present_rv_new)
  ...
  ggsurvplot(my_survfit, data = data_x,            # Add p-value
             conf.int = TRUE,           # Add confidence intervals
             risk.table = TRUE,         # Add risk table
             tables.theme = theme_cleantable(),  # Clean theme for tables
             palette = "jco",           # Change color palette
             legend.labs = c("Negative", "Positive"),  # Change legend labels
             legend.title = "Covid Status",      # Change legend title
             xlab = "Time (days)",      # Change x-axis label
             ylab = "Survival Probability",            # Change y-axis label
             title = "Survival Curves of Secondary Respiratory Infection by SARS-CoV-2 Status",  # Change plot title
             ggtheme = theme_bw()      # Change plot theme
  )
}

# Define survival function
survival_function_rv<- function(data_x, strata_x) {
  # Create survival object
  ...
  ggsurvplot(my_survfit, data = data_x,            # Add p-value
             ...
             palette = "jco",           # Change color palette
             legend.labs = c("Negative", "Positive"),  # Change legend labels
             legend.title = "RV Status",      # Change legend title
             xlab = "Time (days)",      # Change x-axis label
             ylab = "Survival Probability",            # Change y-axis label
             title = "Survival Curves of Secondary Respiratory Infection by Rhinovirus Status",  # Change plot title
             ggtheme = theme_bw()      # Change plot theme
  )
}
...
plot_covid = survival_function_Cov(data_x = tt, strata_x = "present_sars_cov_2_n")

plot_rv = survival_function_rv(data_x = Rhinovirus_TimeDependent_cov, strata_x = "present_rv_n")
...
Incidence.Calculator.Secondary<- function(dataset) {

  # Extract selected columns
  Dt <- dataset[, c('individual', 'enrollment_date', 'followupDate', 'start_adjusted', 'stop_adjusted', 'gap_adjusted', "Secondary_Group")]
  # Create person-day column
  Dt$Person_day <- NA
  Dt$Year_Month <- format(as.Date(Dt$followupDate), "%Y-%m")
  Dt = Dt %>%
    group_by(individual) %>%
    mutate(Person_day = as.integer(followupDate - enrollment_date),
           Secondary_Count = cumsum(Secondary_Group))
  # Calculation of Incidence
  Category <- unique(Dt$Year_Month)[order(unique(Dt$Year_Month))]
  
  Incidence.Calendar = Dt %>%
    group_by(Year_Month) %>%
    summarise(Secondary_Count= sum(Secondary_Count),
              Person_day = sum(Person_day)) %>%
    mutate(Incidence = Secondary_Count/Person_day, 
           Alternative = 1:length(Incidence))
...
# Function to calculate incidence rate
Incidence.Calculator.Primary<- function(dataset, pathogen) {

  # Extract selected columns
  Dt <- dataset[, c('individual', 'enrollment_date', 'followupDate', 'start_adjusted', 'stop_adjusted', 'gap_adjusted', pathogen)]
  # Create person-day column
  Dt$Person_day <- NA
  Dt$Year_Month <- format(as.Date(Dt$followupDate), "%Y-%m")
  Dt = Dt %>%
    group_by(individual) %>%
    mutate(Person_day = as.integer(followupDate - enrollment_date),
           Primary_Count = cumsum(!!as.symbol(pathogen)))
  # Calculation of Incidence
  Category <- unique(Dt$Year_Month)[order(unique(Dt$Year_Month))]
  
  Incidence.Calendar = Dt %>%
    group_by(Year_Month) %>%
    summarise(Primary_Count= sum(Primary_Count),
              Person_day = sum(Person_day)) %>%
    mutate(Incidence = Primary_Count/Person_day, 
           Alternative = 1:length(Incidence))
  
  return(Incidence.Calendar)
}

Rhinovirus_TimeDependent
...
### (7.2) Incidence plot for secondary infections

```{r}

incidence_rhino <- Incidence.Calculator.Secondary(Rhinovirus_TimeDependent)

# Covid
incidence_covid =  Incidence.Calculator.Secondary(Covid_TimeDependent)
#ADV
incidence_adv =  Incidence.Calculator.Secondary(Adenovirus_TimeDependent)
#Rsv
incidence_rsv =  Incidence.Calculator.Secondary(RSV_TimeDependent)
#Influenza A
incidence_iav =  Incidence.Calculator.Secondary(IAV_TimeDependent)
...
smoothIncidence_covid <- ss(x = incidence_covid$Alternative, y = incidence_covid$Incidence, df = 4)

# Non-parametric smooth curve for estimating incidence rate over time for Rhinovirus
smoothIncidence_rhino <- ss(x = incidence_rhino$Alternative, y = incidence_rhino$Incidence, df = 4)

# Non-parametric smooth curve for estimating incidence rate over time for Adenovirus
smoothIncidence_adv <- ss(x = incidence_adv$Alternative, y = incidence_adv$Incidence, df = 4)

# Non-parametric smooth curve for estimating incidence rate over time for RSV
smoothIncidence_rsv <- ss(x = incidence_rsv$Alternative, y = incidence_rsv$Incidence, df = 4)

# Non-parametric smooth curve for estimating incidence rate over time for Influenza A
smoothIncidence_iav <- ss(x = incidence_iav$Alternative, y = incidence_iav$Incidence, df = 4)
...
# Create x-axis label
xLabel <- unique(incidence_rhino$Year_Month)
...
Dt.Plot_combined %>%
  ...
  scale_color_manual(name = "Secondary to", values = c("Covid Primary" = "orange", "RV Primary" = "blue", "Adenovirus Primary" = "green", "RSV Primary" = "red", "Influenza A Primary"= "purple",
                                 "Influenza B Primary"= "black")) + 
  theme_bw() +
  scale_x_continuous(expand = c(0.005, 0),
                     breaks = seq(1, length(xLabel)), # Adjust 'by' to show fewer labels
                     labels = xLabel[seq(1, length(xLabel))]) +
  theme(panel.background = element_blank(),                    
        axis.line.x = element_line(),                               
        axis.line.y = element_line(),
...
### (7.3) Primary Incidence

```{r}
#RV
incidence_rhino.primary <- Incidence.Calculator.Primary(DtAdjusted, "present_rv_n")
#Covid
incidence_covid.primary = Incidence.Calculator.Primary(DtAdjusted, "present_sars_cov_2_n")
#ADV
incidence_adv.primary =  Incidence.Calculator.Primary(DtAdjusted, "present_adv_n")
#Rsv
incidence_rsv.primary =  Incidence.Calculator.Primary(DtAdjusted, "present_rsv_n")
#Influenza A
incidence_iav.primary =  Incidence.Calculator.Primary(DtAdjusted, "present_iav_n")
...
# Smooth curves for each virus
smoothIncidence_rhino_primary <- smooth.spline(incidence_rhino.primary$Alternative, incidence_rhino.primary$Incidence, df = 4)
smoothIncidence_covid_primary <- smooth.spline(incidence_covid.primary$Alternative, incidence_covid.primary$Incidence, df = 8)
smoothIncidence_adv_primary <- smooth.spline(incidence_adv.primary$Alternative, incidence_adv.primary$Incidence, df = 4)
smoothIncidence_rsv_primary <- smooth.spline(incidence_rsv.primary$Alternative, incidence_rsv.primary$Incidence, df = 4)
smoothIncidence_iav_primary <- smooth.spline(incidence_iav.primary$Alternative, incidence_iav.primary$Incidence, df = 4)

# Create data frames for plotting
Dt.Plot_rhino_primary <- data.frame('x' = smoothIncidence_rhino_primary$x, 'y' = ifelse(smoothIncidence_rhino_primary$y < 0, 0, smoothIncidence_rhino_primary$y), 'Virus' = 'Rhinovirus')
Dt.Plot_covid_primary <- data.frame('x' = smoothIncidence_covid_primary$x, 'y' = ifelse(smoothIncidence_covid_primary$y < 0, 0, smoothIncidence_covid_primary$y), 'Virus' = 'Covid-19')
Dt.Plot_adv_primary <- data.frame('x' = smoothIncidence_adv_primary$x, 'y' = ifelse(smoothIncidence_adv_primary$y < 0, 0, smoothIncidence_adv_primary$y), 'Virus' = 'Adenovirus')
Dt.Plot_rsv_primary <- data.frame('x' = smoothIncidence_rsv_primary$x, 'y' = ifelse(smoothIncidence_rsv_primary$y < 0, 0, smoothIncidence_rsv_primary$y), 'Virus' = 'Respiratory Syncytial Virus')
Dt.Plot_iav_primary <- data.frame('x' = smoothIncidence_iav_primary$x, 'y' = ifelse(smoothIncidence_iav_primary$y < 0, 0, smoothIncidence_iav_primary$y), 'Virus' = 'Influenza A')

# Combine the data frames
Dt.Plot_combined_primary <- rbind(Dt.Plot_rhino_primary, Dt.Plot_covid_primary, Dt.Plot_adv_primary, Dt.Plot_rsv_primary, Dt.Plot_iav_primary)

# Create x-axis label
xLabel <- unique(incidence_adv.primary$Year_Month)
# Round digit function
scaleFUN <- function(x){sprintf("%.2f", x)}

# Plot the combined incidence data with different colors for each pathogen
Dt.Plot_combined_primary %>%
  ggplot(aes(x = x, y = y, color = Virus)) + 
  geom_line(alpha = 0.75) + 
  xlab('Calendar Time (Y-M)') + 
  ylab('Incident Rate (Number of Detection / Person-day)') + 
  scale_color_manual(values = c("Covid-19" = "orange", "Rhinovirus" = "blue", "Adenovirus" = "green", "Respiratory Syncytial Virus" = "red", "Influenza A" = "purple")) + 
  theme_bw() +
  scale_x_continuous(expand = c(0.005, 0),
                     breaks = seq(1, length(xLabel)), # Adjust 'by' to show fewer labels
                     labels = xLabel[seq(1, length(xLabel))]) +
...
# Function to calculate incidence rate for primary column
Incidence.Calculator.Primary <- function(dataset) {
  # Identify the primary column starting with "present_"
  primary_col <- grep("^present_", colnames(dataset), value = TRUE)
  if (length(primary_col) != 1) {
    stop("There should be exactly one column starting with 'present_'")
  }
  
  # Rename the primary column to 'present'
  colnames(dataset)[colnames(dataset) == primary_col] <- 'present'
  
  # Extract selected columns
  Dt <- dataset[, c('individual', 'enrollment_date', 'followupDate', 'start_adjusted', 'stop_adjusted', 'gap_adjusted', 'present')]
  
  # Create person-day column
  Dt$Person_day <- NA
  
  # Calculate person-day for each individual
  Dt$Person_day <- Dt$stop_adjusted - Dt$start_adjusted
  # Person-day calculation
  id <- unique(Dt$individual)
  Dt2 <- data.frame()
  for (i in 1:length(id)) {
    Trans <- Dt[Dt$individual == id[i], ]
    n <- nrow(Trans)
    for (j in 1:n) {
      if (Trans[j, 'present'] == 1) {
  ...
  Dt$Year_Month <- format(as.Date(Dt$followupDate), "%Y-%m-%d")
  # Calculation of Incidence
  Category <- unique(Dt$Year_Month)[order(unique(Dt$Year_Month))]
  Incidence.Calendar <- data.frame('Alternative' = 1:length(Category), 'Time' = Category, 'Primary_Count' = 0, 'Person_day' = 0)
  
  n.Dt <- nrow(Dt)
  for (i in 1:n.Dt) {
    if (Dt[i, 'present'] == 1) {
      loca <- match(Dt[i, 'followupDate'], Category)
      Incidence.Calendar[loca, 'Primary_Count'] <- Incidence.Calendar[loca, 'Primary_Count'] + 1
      Incidence.Calendar[loca, 'Person_day'] <- Incidence.Calendar[loca, 'Person_day'] + Dt[i, 'Person_day']
    }
  }
  Incidence.Calendar$Incidence <- Incidence.Calendar$Primary_Count / Incidence.Calendar$Person_day
...
```{r}
# Convert data to survival object
surv_obj <- Surv(Rhinovirus_TimeDependent$start_adjusted, Rhinovirus_TimeDependent$present_rv_n)

# Create a dataframe with calendar time intervals

# Calculate person-time at risk for each interval
person_time <- sapply(calendar_intervals, function(interval) {
  sum(data$stop_adjusted[Rhinovirus_TimeDependent$start_adjusted <= interval & Rhinovirus_TimeDependent$stop_adjusted >= interval])
})

# Calculate the number of events (cases) within each interval
event_counts <- sapply(calendar_intervals, function(interval) {
  sum(Rhinovirus_TimeDependent$present_rv_n[Rhinovirus_TimeDependent$start_adjusted <= interval & Rhinovirus_TimeDependent$stop_adjusted >= interval])
})

# Calculate incidence rate for each interval
incidence_rate <- event_counts / person_time

# Create a dataframe for plotting
incidence_df <- data.frame(
  Calendar_Time = calendar_intervals,
  Incidence_Rate = incidence_rate
)

# Plot the incidence rate over calendar time
ggplot(incidence_df, aes(x = Calendar_Time, y = Incidence_Rate)) +
  geom_line() +
  xlab("Calendar Time") +
  ylab("Incidence Rate") +
  ggtitle("Incidence Rate Over Calendar Time")
...
table_demog <- filtered_data %>%
  summarize(
    ...
    Influenza_B_N = sum(present_ibv_n > 0),
        Co_Detection = sum(rowSums(select(., present_rv_n, present_sars_cov_2_n, present_rsv_n, present_adv_n, present_iav_n, present_ibv_n)) > 2))
...
# generate function secondary infections for table 2

generate_summary_table <- function(data) {
  var_name <- sub("_cov$", "", deparse(substitute(data)))
  
  tbl <- data %>%
    ungroup() %>%
    select(-individual, -sample) %>%
    tbl_summary(by = var_name) %>%
    modify_header(label = "**Variable**") %>%
...
generate_summary_table(Rhinovirus_TimeDependent_cov)
```
...
## (9) CT value subsequent to infection analysis
...
Covid_TimeDependent_cov %>%
  complete.cases(ct_SARS_CoV_2)
Covid_TimeDependent_cov_t = Covid_TimeDependent_cov[!is.na(Covid_TimeDependent_cov$ct_Rhinovirus), ]

# adjusted and unadjusted model
model1 = geeglm(ct_SARS_CoV_2 ~ Rhinovirus_TimeDependent + symptoms+ agebin , data = Rhinovirus_TimeDependent_cov , corstr = "ar1", id = individual, family = gaussian())
...
Covid_TimeDependent_cov_t = Covid_TimeDependent_cov %>%
  filter(sample != "3b191002-a783-457e-980e-a8df2db0f255")

model2 = geeglm(ct_Rhinovirus ~ Covid_TimeDependent + symptoms + sex+ agebin+ ns(date, df = 3), data = Covid_TimeDependent_cov, corstr = "ar1", id = individual, family = gaussian())

tab_model(model2)

# Mixed Effects models
# RV CT
mix_model_rv <- lmer(ct_Rhinovirus ~ Covid_TimeDependent + symptoms +sex+agebin+ ns(date, df = 8)+(1 | individual), REML = T, lmerControl(optimizer ='optimx', optCtrl=list(method='bobyqa')), data = Covid_TimeDependent_cov)
# Covid ct
mix_model_cv <- lmer(ct_SARS_CoV_2 ~ Rhinovirus_TimeDependent +ns(date, df = 8)+symptoms + agebin+sex+ (1 | individual) , REML = T, lmerControl(optimizer = "optimx", optCtrl=list(method='bobyqa')), data = Rhinovirus_TimeDependent_cov)
...
ct_cv_full <- DtAdjusted %>%
  ...
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "CV Ct based on Symptoms") +
  sm_corr_theme() +
...
ct_rv_full <- DtAdjusted %>%
  ...
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "RV Ct based on Symptoms") +
  sm_corr_theme() +
  scale_fill_manual(values = sm_color('blue','orange'))
...
ct_rsv_full <- DtAdjusted %>%
  ...
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "RSV Ct based on Symptoms") +
  sm_corr_theme() +
  scale_fill_manual(values = sm_color('blue','orange'))
...
ct_adeno_full <- DtAdjusted %>%
  ...
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "Adenovirus Ct based on Symptoms") +
  sm_corr_theme() +
  scale_fill_manual(values = sm_color('blue','orange'))
...
ct_iav_full <- DtAdjusted %>%
  ...
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "IAV Ct based on Symptoms") +
  sm_corr_theme() +
  scale_fill_manual(values = sm_color('blue','orange'))
...
ct_ibv_full <- DtAdjusted %>%
  ...
  geom_point()+
  sm_raincloud() +
  labs(x = "Symptoms", y = "Cycle Threshold (Ct)", title = "IBV Ct based on Symptoms") +
  sm_corr_theme() +
  scale_fill_manual(values = sm_color('blue','orange'))
...
ct_fig = (ct_cv_full + ct_rv_full) / (ct_adeno_full + ct_rsv_full)

ggsave(filename = "ct_fig.png", plot =ct_fig, width = 8, height = 6 )
...
Rhinovirus_TimeDependent_cov %>%
  ...
  labs(x = "Time Periods",
       y = "CT Values (Covid)",
       title = "Distribution of CT Values Subsequent to RV infection",
       color = "Virus") +
