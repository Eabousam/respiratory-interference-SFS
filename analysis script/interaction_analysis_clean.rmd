...
sum(length(unique(encounter_rv_cv_pairs_conf$individual)))
```
...
Covid_TimeDependent$Covid_TimeDependent <- factor(Covid_TimeDependent$Covid_TimeDependent, 
                                                  #'30 < Day <= 60',
                                                            levels = c("No", 'Day <= 14','14 < Day <= 30', '30 < Day <= 60' ,"60 < Day <= 180",  "180 < Day <= 300", "300 < Day <= 420", "Day > 420"))
...
# Formula Rhinovirus adjusted
fomular <- Surv(start_adjusted, stop_adjusted, Secondary_Group) ~ present_sars_cov_2_n + sex+ agebin + symptoms
# Rhinovirus model
model <- coxph(fomular,cluster = individual, data = Covid_TimeDependent_cov)
summary(model)
...
plot_covid = survival_function_Cov(data_x = Covid_TimeDependent_cov, strata_x = "present_sars_cov_2_n")
...
## (8) CT value subsequent to infection analysis
